...
const Card = ({ title, imageSrc, description }) => {
  return (
    <div className="bg-gray-800 border-2 border-orange-500 text-white p-6 rounded-lg shadow-md mb-6 w-[50rem]"> {/* Aumentando a largura */}
      ...
      <div className='flex items-center justify-center p-8'> 
        <div className="bg-orange-500 flex justify-center mb-4 border-2 rounded-xl p-4 " style={{ borderColor: '#5D8B29' }}>
          <img src={imageSrc} alt={title} className="w-24 h-24 object-contain" /> {/* Aumentando o tamanho da imagem, se desejar */}
        </div>
        <p className="text-sm text-center w-64 px-6">{description}</p> {/* Ajustando a largura do texto para ocupar todo o espaço */}
      </div>
    </div>
  );
};
...
const Card2 = ({ title, imageSrc, description }) => {
  return (
    <div className="bg-orange-500 border-2 border-green-400 text-white p-6 rounded-lg shadow-md mb-6 w-96 h-72 flex flex-col items-center justify-center">
